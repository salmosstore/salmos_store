(() => {
  ...
  function miniStockSummary(summary={}){
    ...
    const productCards=products.map(p=>{
      ...
      return `<article class="dashboard-stock-product"><div class="dashboard-stock-product-media">${p.imageUrl?`<img src="${escapeHtml(p.imageUrl)}" alt="${escapeHtml(p.name||'Producto')}" loading="lazy">`:'<span>SALMOS</span>'}</div><div class="dashboard-stock-product-copy"><strong>${escapeHtml(p.name||'Producto')}</strong><small>${escapeHtml(p.category||'Sin categoría')}</small><div>${counts}</div></div></article>`;
    }).join('');
    return `<div class="admin-card dashboard-stock-summary"><div class="stock-size-summary"><div class="stock-count-chip total"><span>Todo</span><strong>${total}</strong></div>${categories.map(x=>`<div class="stock-count-chip"><span>${escapeHtml(x.name||'Sin categoría')}</span><strong>${Number(x.units)||0}</strong></div>`).join('')}</div><div class="dashboard-stock-products">${productCards||'<div class="empty-state"><strong>Sin productos con stock.</strong></div>'}</div></div>`
  }
