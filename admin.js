(() => {
  ...
  function miniStockSummary(summary={}){
    ...
    const categoryBlocks=categories.map(cat=>{
      const rows=breakdown.filter(x=>x.category===cat.name);
      const aggregate=(key)=>{const m=new Map();rows.forEach(r=>{const v=String(r[key]||'—');m.set(v,(m.get(v)||0)+(Number(r.units)||0))});return [...m.entries()].filter(([v])=>v!=='—').map(([v,n])=>`${escapeHtml(v)}: <b>${n}</b>`).join(' · ')};
      const sizes=aggregate('size'),colors=aggregate('color'),fits=aggregate('fit');
      return `<div class="dashboard-stock-category"><div class="dashboard-stock-category-head"><strong>${escapeHtml(cat.name||'Sin categoría')}</strong><span>${Number(cat.units)||0}</span></div>${sizes?`<small><b>Talles:</b> ${sizes}</small>`:''}${colors?`<small><b>Colores:</b> ${colors}</small>`:''}${fits?`<small><b>Cortes:</b> ${fits}</small>`:''}</div>`;
    }).join('');
    return `<div class="admin-card dashboard-stock-summary"><div class="stock-size-summary"><div class="stock-count-chip total"><span>Total</span><strong>${total}</strong></div>${categories.map(x=>`<div class="stock-count-chip"><span>${escapeHtml(x.name||'Sin categoría')}</span><strong>${Number(x.units)||0}</strong></div>`).join('')}</div>${categoryBlocks?`<div class="dashboard-stock-breakdown">${categoryBlocks}</div>`:''}${stages.length?`<div class="dashboard-stock-stages">${stages.map(x=>`<span class="status ${x.stage==='finished'?'success':'warning'}">${stageTitle(x.stage)}: ${Number(x.units)||0}</span>`).join('')}</div>`:''}</div>`
  }
