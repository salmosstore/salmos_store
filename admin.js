(() => {
  ...
  function bind(){
    ...
    document.addEventListener('click',async e=>{
      ...
      if(e.target.id==='uploadDesignBtn'){const files=[...(qs('#designUploadFiles')?.files||[])];if(!files.length){toast('Elegí al menos un archivo.','error');return}const scope=qs('#designUploadScope')?.value||'clients';const kind=scope==='mixed'?'sheet':(qs('#designUploadKind')?.value||'individual');try{e.target.disabled=true;const uploaded=[];for(const file of files){const fd=new FormData();fd.append('file',file);fd.append('scope',scope);fd.append('kind',kind);fd.append('name',(qs('#designUploadName')?.value||'').trim()||(files.length===1?file.name.replace(/\.[^.]+$/,''):'') );fd.append('note',qs('#designUploadNote')?.value||'');fd.append('widthCm',qs('#designUploadWidth')?.value||'');fd.append('heightCm',qs('#designUploadHeight')?.value||'');fd.append('printMaterialType',qs('#designUploadPrintType')?.value||'dtf_textile');const saved=await api('/api/admin/design-assets',{method:'POST',body:fd});if(saved.item)uploaded.push(saved.item)}toast('Diseño/s guardado/s sin modificar','success');qs('#designUploadDialog')?.close();state.costingLoaded=false;if(state.designUploadContext==='production'){const data=await api('/api/admin/design-assets');state.designAssets=data.items||[];for(const d of uploaded)if(d.kind==='individual'&&!state.productionDesignsSelected.some(x=>Number(x.designAssetId)===Number(d.id)))state.productionDesignsSelected.push({designAssetId:Number(d.id),quantity:1});renderProductionDesignGallery();renderProductionSelectedDesigns();}else await renderDesigns()}catch(err){toast(err.message,'error')}finally{e.target.disabled=false}return}
