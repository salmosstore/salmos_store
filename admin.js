(() => {
  ...
  function setMovementReadOnly(readOnly){
    ...
    qsa('.dialog-body input,.dialog-body select,.dialog-body textarea',form).forEach(el=>el.disabled=readOnly);
  ...
  async function openMovementDialog(id,readOnly=false){
    ...
    form.elements.origin.value=m.origin||'';form.elements.destination.value=m.destination||'';loadMovementPayment(m);form.elements.date.value=String(m.occurred_at||'').slice(0,10);
    ...
    setMovementReadOnly(readOnly||m.source_kind==='purchase');qs('#editViewedMovementBtn').textContent=m.source_kind==='purchase'?'Editar compra':'Editar';qs('#movementDialog').showModal();
  }
  ...
  function designMeasureRow(o={}){
    ...
    return `<div class="design-measure-option-row" data-measure-option data-measure-id="${escapeHtml(id)}"><div class="field"><label>Destino · podés elegir varios</label>${designDestinationPicker(o)}</div><label class="field">Talle (opcional)<input class="input" data-measure-field="size" value="${escapeHtml(o.size||'')}" placeholder="Ej.: L"></label><label class="field">Ancho / largo (cm)<input class="input" data-measure-field="widthCm" type="number" min=".01" step=".01" value="${Number(o.widthCm)||''}" placeholder="30"></label><label class="field">Alto (cm)<input class="input" data-measure-field="heightCm" type="number" min=".01" step=".01" value="${Number(o.heightCm)||''}" placeholder="40"></label><label class="field measure-detail-field">Detalle (opcional)<input class="input" data-measure-field="detail" value="${escapeHtml(o.detail||'')}" placeholder="Ej.: espalda, pecho o mate de 250 ml"></label><button class="icon-btn measure-remove" type="button" data-remove-measure-option aria-label="Quitar opción de medidas">×</button></div>`;
  }
  ...
  function readDesignMeasureOptions(root){
    return qsa('[data-measure-option]',root).flatMap((row,i)=>{
      ...
      return [{id:row.dataset.measureId,destination:destinations.join(' / '),destinations,size,detail,widthCm,heightCm}];
    });
  }
  ...
  function sheetComponents(key){return (state.designSheetDrafts?.[key]||[]).map(c=>{const a=state.designAssets.find(x=>Number(x.id)===Number(c.designAssetId)),o=a&&parseDesignMeasureOptions(a).find(x=>x.id===c.measureOptionId);return {...c,...(o||{}),measureOptionId:c.measureOptionId,name:a?.name||c.name,scope:a?.scope||c.scope}})}
  function sheetMetrics(width,height,components){const total=width*height,used=components.reduce((sum,c)=>sum+(Number(c.widthCm)||0)*(Number(c.heightCm)||0)*(Number(c.quantity)||0),0);return {total,used,waste:Math.max(0,total-used),percent:total>0?Math.max(0,total-used)/total*100:0,valid:total>0&&used>0&&used<=total+.0001}}
  function sheetMetricsHtml(width,height,components){
    const m=sheetMetrics(width,height,components),n=v=>Number(v).toLocaleString('es-AR',{maximumFractionDigits:2});
  ...
  function sheetCompositionEditor(key){return `<section class="sheet-composition" data-sheet-editor="${key}"><h3>Diseños que componen la plancha</h3><p class="muted">Elegí los individuales; sus destinos, talles y medidas se completan automáticamente.</p>${['salmos','clients'].map(scope=>`<h4>${scope==='salmos'?'SALMOS':'Clientes'}</h4><div class="sheet-sample-gallery">${state.designAssets.filter(a=>a.kind==='individual'&&a.scope===scope).map(a=>`<button type="button" class="sheet-sample" data-add-sheet-design="${a.id}" data-sheet-key="${key}" ${parseDesignMeasureOptions(a).length?'':'disabled'} title="${escapeHtml(a.name||a.file_name)}"><span>${String(a.mime_type||'').startsWith('image/')?`<img src="${apiUrl(`/api/admin/design-assets/${a.id}/file`)}" alt="${escapeHtml(a.name||a.file_name)}" loading="lazy">`:'Archivo'}</span><small>${escapeHtml(a.name||a.file_name)}</small>${parseDesignMeasureOptions(a).length?'':'<small>Faltan medidas</small>'}</button>`).join('')||'<p class="muted">Sin diseños individuales.</p>'}</div>`).join('')}<div data-sheet-selected></div><div data-sheet-metrics></div></section>`}
  function renderSheetSelected(key){
    const root=sheetRoot(key),host=qs('[data-sheet-selected]',root);if(!host)return;const components=sheetComponents(key);
    host.innerHTML=components.map((c,i)=>{const asset=state.designAssets.find(a=>Number(a.id)===Number(c.designAssetId)),options=asset?parseDesignMeasureOptions(asset):[];return `<div class="sheet-component-row" data-sheet-component="${i}" data-sheet-key="${key}"><strong>${escapeHtml(c.name||'Diseño')}</strong><label class="field">Opción de medida<select class="select" data-sheet-measure>${options.map(o=>`<option value="${escapeHtml(o.id)}" ${o.id===c.measureOptionId?'selected':''}>${escapeHtml(designMeasureLabel(o))}</option>`).join('')}</select></label><label class="field">Cantidad<input class="input" data-sheet-quantity type="number" min="1" step="1" value="${Number(c.quantity)||1}"></label><div class="admin-actions"><button type="button" class="icon-btn" data-sheet-other-size="${i}" data-sheet-key="${key}" aria-label="Agregar otra medida del diseño">+</button><button type="button" class="icon-btn" data-remove-sheet-component="${i}" data-sheet-key="${key}" aria-label="Quitar diseño">×</button></div><small class="field-help">${escapeHtml(designMeasureLabel(c))}</small></div>`}).join('')||'<p class="muted">Todavía no elegiste diseños.</p>';
    qsa('[data-add-sheet-design]',root).forEach(b=>b.classList.toggle('selected',components.some(c=>Number(c.designAssetId)===Number(b.dataset.addSheetDesign))));updateSheetMetrics(key);
  }
  ...
  function addSheetDesign(key,id,another=false){
    ...
    if(!option){toast('Este diseño no tiene otra medida disponible.','error');return;}
  ...
  function sheetPayload(key){const kind=sheetKind(key),size=sheetSize(key);return {widthCm:kind==='sheet'?size.width:0,heightCm:kind==='sheet'?size.height:0,measureOptions:kind==='sheet'?[]:readDesignMeasureOptions(sheetRoot(key)),components:kind==='sheet'?(state.designSheetDrafts?.[key]||[]).map(c=>({designAssetId:Number(c.designAssetId),measureOptionId:c.measureOptionId,quantity:Number(c.quantity)})):[]};}
  function openDesignUpload(context='designs'){
    state.designUploadContext=context;state.designSheetDrafts??={};state.designSheetDrafts.upload=[];
    for(const id of ['designUploadName','designUploadNote','designUploadWidth','designUploadHeight','designUploadFiles'])qs('#'+id).value='';
    ...
    qs('#designUploadMeasures').innerHTML=designMeasureEditor('upload');qs('#designUploadComposition').innerHTML='';refreshDesignKind('upload');qs('#designUploadDialog').showModal();
  }
  async function uploadDesignFiles(button){
    ...
    const scope=qs('#designUploadScope').value,kind=scope==='mixed'?'sheet':qs('#designUploadKind').value,detail=sheetPayload('upload');
    ...
    try{for(const file of files){const fd=new FormData();fd.append('file',file);fd.append('scope',scope);fd.append('kind',kind);fd.append('name',(qs('#designUploadName').value||'').trim()||(files.length===1?file.name.replace(/\.[^.]+$/,''):''));fd.append('note',qs('#designUploadNote').value||'');fd.append('printMaterialType',qs('#designUploadPrintType').value);for(const [key,value] of Object.entries(detail))fd.append(key,typeof value==='object'?JSON.stringify(value):String(value));const saved=await api('/api/admin/design-assets',{method:'POST',body:fd});if(saved.item)uploaded.push(saved.item);}
      qs('#designUploadDialog').close();state.costingLoaded=false;toast('Diseño/s guardado/s','success');
      if(state.designUploadContext==='production'){const data=await api('/api/admin/design-assets');state.designAssets=data.items||[];renderProductionDesignGallery();renderProductionSelectedDesigns();state.productionMeasureQueue=uploaded.filter(d=>d.kind==='individual').map(d=>Number(d.id));if(state.productionMeasureQueue.length)openProductionMeasureDialog(state.productionMeasureQueue.shift());}else await renderDesigns();
    }finally{button.disabled=false;}
  }
  function designCard(a){const preview=`/api/admin/design-assets/${a.id}/file`;return `<article class="design-gallery-card" data-design-card="${a.id}" draggable="true">${a.kind==='sheet'?`<label class="design-sheet-check" title="Seleccionar plancha"><input type="checkbox" data-design-sheet="${a.id}" ${state.selectedDesignSheets?.has(Number(a.id))?'checked':''}><span>✓</span></label>`:''}<button type="button" class="design-gallery-thumb checkerboard" data-preview-design="${a.id}" title="${escapeHtml(a.name||a.file_name)}">${String(a.mime_type||'').startsWith('image/')?`<img draggable="false" src="${preview}" alt="${escapeHtml(a.name||a.file_name)}" loading="lazy">`:`<div class="design-file-placeholder">${escapeHtml((a.file_name||'ARCHIVO').split('.').pop().toUpperCase())}</div>`}</button><div class="design-reorder-controls"><button type="button" class="icon-btn" data-move-design="-1" data-design-id="${a.id}" aria-label="Mover a la izquierda">‹</button><span>Ordenar</span><button type="button" class="icon-btn" data-move-design="1" data-design-id="${a.id}" aria-label="Mover a la derecha">›</button></div></article>`}
  ...
  async function renderDesigns(){
    state.designListOpen??={};qsa('[data-design-list]').forEach(d=>state.designListOpen[d.dataset.designList]=d.open);
    const d=await api('/api/admin/design-assets');state.designAssets=d.items||[];
    ...
    qs('#adminContent').innerHTML=`<div class="admin-section-head design-library-toolbar"><div><h2 style="margin:0">Biblioteca de diseños</h2><p class="muted" style="margin:4px 0 0">Tocá un diseño para ver sus destinos y medidas.</p></div><div class="admin-actions"><button class="btn btn-primary" type="button" id="openDesignUploadBtn">+ Subir diseño</button><button class="btn btn-ghost" type="button" id="openDesignEmailBtn">Enviar planchas por mail</button></div></div>${designGroup('SALMOS',salmos,'salmos')}${designGroup('Clientes',clients,'clients')}${designSection('Planchas mixtas',mixed,'mixed')}`;
  }
  function designDialogPayload(){const root=qs('#designDetailEditor');return {scope:qs('[data-design-scope]',root)?.value||'salmos',kind:qs('[data-design-kind]',root)?.value||'individual',name:qs('[data-design-name]',root)?.value||'',note:qs('[data-design-note]',root)?.value||'',printMaterialType:qs('[data-design-print-type]',root)?.value||'dtf_textile',printed:Boolean(qs('[data-design-printed]',root)?.checked),...sheetPayload('detail')}}
  function openDesignPreview(id){
    const a=state.designAssets.find(x=>Number(x.id)===Number(id));if(!a)return;state.activeDesignId=Number(a.id);state.designSheetDrafts??={};state.designSheetDrafts.detail=(a.components||[]).map(c=>({...c}));
    ...
    const composition=a.kind==='sheet'?`<h3>Superficie: ${Number(a.width_cm)||'—'} × ${Number(a.height_cm)||'—'} cm</h3>${a.components?.length?`<ul class="design-measure-summary">${a.components.map(c=>`<li><strong>${escapeHtml(c.name)} × ${c.quantity}</strong><br>${escapeHtml(designMeasureLabel(c))}</li>`).join('')}</ul>${sheetMetricsHtml(Number(a.width_cm),Number(a.height_cm),a.components)}`:'<p class="muted">Agregá la composición para calcular el desperdicio de esta plancha.</p>'}`:measures.length?`<ul class="design-measure-summary">${measures.map(o=>`<li>${escapeHtml(designMeasureLabel(o))}</li>`).join('')}</ul>`:'';
  ...
  async function moveDesignCard(id,direction,targetId=0){
    ...
    try{await api('/api/admin/design-assets/order',{method:'PATCH',body:JSON.stringify({scope:asset.scope,kind:asset.kind,ids:group.map(a=>Number(a.id))})});let i=0;state.designAssets=state.designAssets.map(a=>a.scope===asset.scope&&a.kind===asset.kind?group[i++]:a);
      const card=qs(`[data-design-card="${id}"]`),gallery=card?.closest('.design-grid');if(gallery){gallery.innerHTML=group.map(designCard).join('');qs(`[data-design-card="${id}"] [data-move-design]`,gallery)?.focus({preventScroll:true});}
  ...
  function applyProductionMeasure(){
    ...
    const existing=state.productionDesignsSelected.find(x=>Number(x.designAssetId)===id),entry={designAssetId:id,quantity:existing?.quantity||1,widthCm:option.widthCm,heightCm:option.heightCm,measureOptionId:option.id,measureLabel:designMeasureLabel(option)};
    if(existing)Object.assign(existing,entry);else state.productionDesignsSelected.push(entry);
  ...
  function bind(){
    ...
    document.addEventListener('change',async e=>{
      ...
      if(e.target.id==='designUploadScope'){const kind=qs('#designUploadKind');if(kind){if(e.target.value==='mixed'){kind.value='sheet';kind.disabled=true}else kind.disabled=false}refreshDesignKind('upload');}
    ...
    document.addEventListener('click',async e=>{
      ...
      const removeSheet=e.target.closest('[data-remove-sheet-component]');if(removeSheet){const key=removeSheet.dataset.sheetKey;state.designSheetDrafts[key].splice(Number(removeSheet.dataset.removeSheetComponent),1);renderSheetSelected(key);return;}
      ...
      const changeMeasure=e.target.closest('[data-change-production-measure]');if(changeMeasure){openProductionMeasureDialog(Number(changeMeasure.dataset.changeProductionMeasure));return;}
