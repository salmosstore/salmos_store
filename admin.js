(() => {
  ...
  function miniStockSummary(summary={}){
    ...
    return `<div class="admin-card dashboard-stock-summary"><div class="stock-size-summary"><div class="stock-count-chip total"><span>Total</span><strong>${total}</strong></div>${categories.map(x=>`<div class="stock-count-chip"><span>${escapeHtml(x.name||'Sin categoría')}</span><strong>${Number(x.units)||0}</strong></div>`).join('')}</div>${stages.length?`<div class="dashboard-stock-stages">${stages.map(x=>`<span class="status ${x.stage==='finished'?'success':'warning'}">${stageTitle(x.stage)}: ${Number(x.units)||0}</span>`).join('')}</div>`:''}</div>`
  }
  ...
  async function renderDashboard(){
    ...
    qs('#adminContent').innerHTML=`
      ...
      <div class="dashboard-quick-grid">
        ...
        <section class="admin-section quick-admin-block"><div class="admin-section-head"><h2>Stock físico</h2><button class="btn btn-ghost" data-go="stock">Abrir</button></div>${miniStockSummary(d.stockSummary||{})}</section>
  ...
  function designCard(a){const uses=parseDesignUses(a);const preview=`/api/admin/design-assets/${a.id}/file`;return `<article class="design-asset-card" data-design-card="${a.id}">
    <div class="design-thumb checkerboard" data-preview-design="${a.id}">${String(a.mime_type||'').startsWith('image/')?`<img src="${preview}" alt="${escapeHtml(a.name||a.file_name)}" loading="lazy">`:`<div class="design-file-placeholder">${escapeHtml((a.file_name||'ARCHIVO').split('.').pop().toUpperCase())}</div>`}</div>
    <div class="design-card-body"><div class="design-card-title"><strong>${escapeHtml(a.name||a.file_name)}</strong>${a.kind==='sheet'?`<label class="sheet-select"><input type="checkbox" data-design-sheet="${a.id}"> seleccionar</label>`:''}</div>
      <div class="design-file-meta">${escapeHtml(a.file_name||'')} · ${Math.max(1,Math.round((Number(a.size_bytes)||0)/1024/1024*10)/10)} MB</div>
      <div class="form-grid design-edit-grid"><div class="field"><label>Nombre</label><input class="input" data-design-name value="${escapeHtml(a.name||'')}"></div><div class="field"><label>Medidas</label><div class="design-measures"><input class="input" data-design-width type="number" min="0" step=".1" value="${Number(a.width_cm)||''}" placeholder="Ancho cm"><span>×</span><input class="input" data-design-height type="number" min="0" step=".1" value="${Number(a.height_cm)||''}" placeholder="Alto cm"></div></div><div class="field full"><label>Nota</label><textarea class="textarea" data-design-note rows="2">${escapeHtml(a.note||'')}</textarea></div></div>
      <div class="design-use-row"><span>Uso:</span><label><input type="checkbox" data-design-use-all ${uses.length===DESIGN_USES.length?'checked':''}> Todo</label>${DESIGN_USES.map(u=>`<label><input type="checkbox" data-design-use value="${escapeHtml(u)}" ${uses.includes(u)?'checked':''}> ${escapeHtml(u)}</label>`).join('')}</div>
      <div class="design-card-actions"><label class="toggle-label"><input type="checkbox" data-design-printed ${Number(a.printed)?'checked':''}> Impreso</label><a class="btn btn-ghost" href="${preview}" target="_blank" rel="noopener">Abrir original</a><button class="btn btn-primary" type="button" data-save-design="${a.id}">Guardar</button><button class="btn btn-danger" type="button" data-delete-design="${a.id}">Eliminar</button></div>
    </div></article>`}
  function designSection(title,items){return `<section class="design-library-section"><div class="admin-section-head"><h3>${title}</h3><span class="muted">${items.length} archivo${items.length===1?'':'s'}</span></div><div class="design-grid">${items.length?items.map(designCard).join(''):'<div class="empty-state"><strong>Sin diseños en esta sección.</strong></div>'}</div></section>`}
  async function renderDesigns(){const d=await api('/api/admin/design-assets');state.designAssets=d.items||[];const clients=state.designAssets.filter(x=>x.scope==='clients'),salmos=state.designAssets.filter(x=>x.scope==='salmos'),mixed=state.designAssets.filter(x=>x.scope==='mixed');qs('#adminContent').innerHTML=`
    <div class="admin-section-head"><div><h2 style="margin:0">Biblioteca de diseños</h2><p class="muted" style="margin:4px 0 0">Los archivos se guardan originales, sin recomprimir ni quitar transparencias.</p></div></div>
    <section class="settings-card design-upload-card"><h3>Subir diseño</h3><div class="form-grid"><div class="field"><label>Sección</label><select class="select" id="designUploadScope"><option value="clients">Clientes</option><option value="salmos">SALMOS</option><option value="mixed">Planchas mixtas</option></select></div><div class="field"><label>Tipo</label><select class="select" id="designUploadKind"><option value="individual">Individual</option><option value="sheet">Plancha</option></select></div><div class="field"><label>Nombre (opcional)</label><input class="input" id="designUploadName" placeholder="Si subís varios usa el nombre del archivo"></div><div class="field"><label>Medidas</label><div class="design-measures"><input class="input" id="designUploadWidth" type="number" min="0" step=".1" placeholder="Ancho cm"><span>×</span><input class="input" id="designUploadHeight" type="number" min="0" step=".1" placeholder="Alto cm"></div></div><div class="field full"><label>Nota</label><textarea class="textarea" id="designUploadNote" rows="2"></textarea></div><div class="field full"><label>Archivos</label><input class="input" id="designUploadFiles" type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml,application/pdf,.tif,.tiff" multiple><small class="field-help">Se sube el archivo binario tal cual. Para DTF, preferí PNG/TIFF original.</small></div></div><div class="admin-actions" style="margin-top:12px"><button class="btn btn-primary" type="button" id="uploadDesignBtn">Subir sin modificar</button></div></section>
    <section class="settings-card design-email-card"><div><h3>Enviar planchas por mail</h3><p class="muted">Marcá varias planchas. El correo envía enlaces de descarga del archivo original, así no se pierde calidad y no hay límite de adjuntos pesados.</p></div><div class="form-grid"><div class="field"><label>Email destinatario</label><input class="input" id="designEmailTo" type="email" placeholder="impresion@..."></div><div class="field full"><label>Mensaje (opcional)</label><textarea class="textarea" id="designEmailMessage" rows="2"></textarea></div></div><button class="btn btn-primary" type="button" id="emailDesignSheetsBtn">Enviar planchas seleccionadas</button></section>
  ...
  function designCardPayload(card){return {name:qs('[data-design-name]',card)?.value||'',note:qs('[data-design-note]',card)?.value||'',widthCm:Number(qs('[data-design-width]',card)?.value)||0,heightCm:Number(qs('[data-design-height]',card)?.value)||0,printed:Boolean(qs('[data-design-printed]',card)?.checked),uses:qsa('[data-design-use]:checked',card).map(x=>x.value)}}
  function openDesignPreview(id){const a=state.designAssets.find(x=>Number(x.id)===Number(id));if(!a)return;const stage=qs('#designPreviewStage');const url=`/api/admin/design-assets/${a.id}/file`;qs('#designPreviewTitle').textContent=a.name||a.file_name;stage.innerHTML=String(a.mime_type||'').startsWith('image/')?`<div class="zoomable-design checkerboard fit" data-toggle-design-zoom><img src="${url}" alt=""></div>`:`<iframe src="${url}" title="Vista previa"></iframe>`;qs('#designPreviewMeta').textContent=`${designScopeLabel(a)} · ${designKindLabel(a)} · ${a.width_cm||'—'} × ${a.height_cm||'—'} cm · ${a.file_name||''}`;qs('#designPreviewDialog').showModal()}
  ...
  function bind(){
    ...
    document.addEventListener('change',async e=>{
      ...
      if(e.target.matches('[data-design-use-all]')){const card=e.target.closest('[data-design-card]');qsa('[data-design-use]',card).forEach(x=>x.checked=e.target.checked)}
