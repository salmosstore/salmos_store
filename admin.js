(() => {
  ...
  function paymentSplit(prefix,total){
    ...
    if(transfer>0&&!origin)throw new Error('Indicá la cuenta de origen de la transferencia.');
    const payments=[];
  ...
  function updatePurchaseTotal(){const subtotal=purchaseSubtotalCents(),surcharge=surchargeData('purchase',subtotal).surcharge,grand=subtotal+surcharge;const a=qs('#purchaseSubtotal'),b=qs('#purchaseTotal');if(a)a.textContent=money(subtotal);if(b)b.textContent=money(grand);updatePurchasePaymentVisibility();}
  ...
  const DESIGN_USES=['Remeras','Tazas','Mates','Gorras','Chombas','Buzos','Vasos','Termos','Bolsos','Otros'];
  ...
  function designMeasureLabel(o){return [o.destination,o.size?`Talle ${o.size}`:'',o.detail,`${Number(o.widthCm)} × ${Number(o.heightCm)} cm`].filter(Boolean).join(' · ')}
  function designMeasureRow(o={}){
    const id=o.id||crypto.randomUUID();
    return `<div class="design-measure-option-row" data-measure-option data-measure-id="${escapeHtml(id)}"><label class="field">Destino<input class="input" data-measure-field="destination" list="designDestinations" value="${escapeHtml(o.destination||'')}" placeholder="Ej.: Remeras o Mate"></label><label class="field">Talle (opcional)<input class="input" data-measure-field="size" value="${escapeHtml(o.size||'')}" placeholder="Ej.: L"></label><label class="field">Ancho / largo (cm)<input class="input" data-measure-field="widthCm" type="number" min=".01" step=".01" value="${Number(o.widthCm)||''}" placeholder="30"></label><label class="field">Alto (cm)<input class="input" data-measure-field="heightCm" type="number" min=".01" step=".01" value="${Number(o.heightCm)||''}" placeholder="40"></label><label class="field measure-detail-field">Detalle (opcional)<input class="input" data-measure-field="detail" value="${escapeHtml(o.detail||'')}" placeholder="Ej.: espalda, pecho o mate de 250 ml"></label><button class="icon-btn measure-remove" type="button" data-remove-measure-option aria-label="Quitar opción de medidas">×</button></div>`;
  }
  function designMeasureEditor(key,options=[]){return `<div class="design-measure-editor" data-measure-editor="${key}"><div class="admin-section-head"><div><strong>Destinos y medidas</strong><p class="muted">Agregá solo las opciones de este diseño. En Producción elegís cuál usar.</p></div><button class="btn btn-ghost" type="button" data-add-measure-option>+ Agregar opción</button></div><div data-measure-rows>${(options.length?options:[{}]).map(designMeasureRow).join('')}</div></div>`}
  function readDesignMeasureOptions(root){
    return qsa('[data-measure-option]',root).flatMap((row,i)=>{
      ...
      const destination=field('destination'),size=field('size'),detail=field('detail'),w=field('widthCm'),h=field('heightCm');
      if(!destination&&!size&&!detail&&!w&&!h)return [];
      const widthCm=Number(w),heightCm=Number(h);
      if(!destination||!Number.isFinite(widthCm)||!Number.isFinite(heightCm)||widthCm<.01||heightCm<.01||widthCm>10000||heightCm>10000)throw new Error(`Completá destino, ancho y alto válidos en la opción ${i+1}.`);
      return [{id:row.dataset.measureId,destination,size,detail,widthCm,heightCm}];
    });
  }
  ...
  function designDialogPayload(){const root=qs('#designDetailEditor');return {scope:qs('[data-design-scope]',root)?.value||'salmos',kind:qs('[data-design-kind]',root)?.value||'individual',name:qs('[data-design-name]',root)?.value||'',note:qs('[data-design-note]',root)?.value||'',widthCm:Number(qs('[data-design-width]',root)?.value)||0,heightCm:Number(qs('[data-design-height]',root)?.value)||0,printMaterialType:qs('[data-design-print-type]',root)?.value||'dtf_textile',printed:Boolean(qs('[data-design-printed]',root)?.checked),measureOptions:readDesignMeasureOptions(root)}}
  function openDesignPreview(id){
    const a=state.designAssets.find(x=>Number(x.id)===Number(id));if(!a)return;state.activeDesignId=Number(a.id);const measures=parseDesignMeasureOptions(a),stage=qs('#designPreviewStage'),url=`/api/admin/design-assets/${a.id}/file`;qs('#designPreviewTitle').textContent=a.name||a.file_name;
  ...
  function openProductionMeasureDialog(id){
    ...
    qs('#productionMeasureDialog').showModal();
  }
  ...
  function bind(){
    ...
    document.addEventListener('change',async e=>{
      ...
      if(e.target.matches('[data-design-scope]')){const kind=qs('[data-design-kind]',qs('#designDetailEditor'));kind.disabled=e.target.value==='mixed';if(kind.disabled)kind.value='sheet';}
    ...
    document.addEventListener('click',async e=>{
      ...
      if(e.target.id==='openDesignUploadBtn'||e.target.id==='productionUploadDesignBtn'){state.designUploadContext=e.target.id==='productionUploadDesignBtn'?'production':'designs';qs('#designUploadFiles').value='';if(state.designUploadContext==='production'){qs('#designUploadScope').value='salmos';qs('#designUploadKind').value='individual';qs('#designUploadKind').disabled=false;for(const id of ['designUploadName','designUploadNote','designUploadWidth','designUploadHeight'])qs('#'+id).value='';}qs('#designUploadMeasures').innerHTML=designMeasureEditor('upload');qs('#designUploadDialog')?.showModal();return}
      ...
      if(e.target.id==='uploadDesignBtn'){const files=[...(qs('#designUploadFiles')?.files||[])];if(!files.length){toast('Elegí al menos un archivo.','error');return}const scope=qs('#designUploadScope')?.value||'clients';const kind=scope==='mixed'?'sheet':(qs('#designUploadKind')?.value||'individual');try{const measureOptions=readDesignMeasureOptions(qs('#designUploadDialog'));e.target.disabled=true;const uploaded=[];for(const file of files){const fd=new FormData();fd.append('file',file);fd.append('scope',scope);fd.append('kind',kind);fd.append('name',(qs('#designUploadName')?.value||'').trim()||(files.length===1?file.name.replace(/\.[^.]+$/,''):'') );fd.append('note',qs('#designUploadNote')?.value||'');fd.append('widthCm',qs('#designUploadWidth')?.value||'');fd.append('heightCm',qs('#designUploadHeight')?.value||'');fd.append('printMaterialType',qs('#designUploadPrintType')?.value||'dtf_textile');fd.append('measureOptions',JSON.stringify(measureOptions));const saved=await api('/api/admin/design-assets',{method:'POST',body:fd});if(saved.item)uploaded.push(saved.item)}toast('Diseño/s guardado/s sin modificar','success');qs('#designUploadDialog')?.close();state.costingLoaded=false;if(state.designUploadContext==='production'){const data=await api('/api/admin/design-assets');state.designAssets=data.items||[];renderProductionDesignGallery();renderProductionSelectedDesigns();state.productionMeasureQueue=uploaded.filter(d=>d.kind==='individual').map(d=>Number(d.id));if(state.productionMeasureQueue.length)openProductionMeasureDialog(state.productionMeasureQueue.shift());}else await renderDesigns()}catch(err){toast(err.message,'error')}finally{e.target.disabled=false}return}
      ...
      if(e.target.id==='newMovementBtn'){await ensureAdminOptionSettings();state.editingMovementId=null;const f=qs('#movementForm');f.reset();f.elements.date.value=today();loadMovementPayment();renderFinanceReasonSelector('');qs('#movementDialogTitle').textContent='Nuevo ingreso / egreso';qs('#movementDialog').showModal();return}
