(() => {
  ...
  function purchaseDefaultItem(type='shirt'){return {materialType:type,customTypeLabel:'',name:type==='dtf_textile'?'DTF textil':type==='dtf_uv'?'DTF UV':'',quantity:1,color:'',size:'',fit:'',materialClass:materialIsGarment(type)?'ADULTOS':'',gender:materialIsGarment(type)?'Unisex':'',widthCm:materialIsDtf(type)?58:0,unitPricePesos:type==='dtf_textile'?10000:type==='dtf_uv'?20000:'',_custom:{}};}
  ...
  function purchaseChoiceField(item,field,label,options,placeholder='Escribí una opción'){
    ...
    const custom=Boolean(item._custom?.[field])||Boolean(value&&!known);
    const selected=custom?'__other__':value;
    return `<div class="field purchase-choice-field purchase-${field}"><label>${escapeHtml(label)}</label><select class="select purchase-choice-select" data-purchase-choice="${field}"><option value="">Elegir...</option>${options.map(v=>`<option value="${escapeHtml(v)}" ${selected===v?'selected':''}>${escapeHtml(v)}</option>`).join('')}<option value="__other__" ${custom?'selected':''}>＋ Otro...</option></select>${custom?`<input class="input purchase-other-input" data-purchase-field="${field}" value="${escapeHtml(value)}" placeholder="${escapeHtml(placeholder)}" autocomplete="off">`:''}</div>`;
  }
  ...
  function purchaseColorField(item,i){
    const options=purchaseColorOptions(),value=normalizeOption(item.color),custom=Boolean(item._custom?.color);
    const label=value||'Elegí un color';
    return `<div class="field purchase-color-field"><label>Color</label><details class="purchase-color-picker"><summary><span class="color-dot" style="--swatch:${escapeHtml(colorSwatch(value||'Rosa'))}"></span><span>${escapeHtml(label)}</span><b>⌄</b></summary><div class="purchase-color-menu">${options.map(v=>`<button type="button" data-purchase-color="${escapeHtml(v)}" data-row="${i}"><span class="color-dot" style="--swatch:${escapeHtml(colorSwatch(v))}"></span><span>${escapeHtml(v)}</span></button>`).join('')}<button type="button" class="color-other" data-purchase-color="__other__" data-row="${i}"><span class="color-dot custom-color-dot"></span><span>Otro color...</span></button></div></details>${custom?`<div class="purchase-custom-color"><input class="input" data-purchase-field="color" value="${escapeHtml(value)}" placeholder="Escribí el color"><input type="color" class="purchase-color-native" data-purchase-color-native data-row="${i}" value="#f29bbc" title="Referencia visual opcional"></div>`:''}</div>`;
  }
  function purchaseTypeField(item,i){
    const type=item.materialType||'shirt',customNew=type==='__other__',opts=currentPurchaseTypes();
    const isKnown=opts.some(([v])=>v===type);
    return `<div class="field purchase-type-field"><label>Tipo</label><select class="select" data-purchase-type-select data-row="${i}">${opts.map(([v,l])=>`<option value="${v}" ${type===v?'selected':''}>${escapeHtml(l)}</option>`).join('')}${!isKnown&&type!=='__other__'?`<option value="${escapeHtml(type)}" selected>${escapeHtml(materialTypeLabel(type))}</option>`:''}<option value="__other__" ${customNew?'selected':''}>＋ Otro...</option></select>${customNew?`<input class="input purchase-other-input" data-purchase-field="customTypeLabel" value="${escapeHtml(item.customTypeLabel||'')}" placeholder="Nombre del nuevo tipo" autocomplete="off">`:''}</div>`;
  }
  function purchaseItemRow(x={},i=0){
    ...
    return `<div class="purchase-item-row ${garment?'is-garment':''} ${dtf?'is-dtf':''}" data-purchase-row="${i}">
      ...
      <div class="purchase-row-actions"><button type="button" class="icon-btn" data-duplicate-purchase-item="${i}" aria-label="Duplicar" title="Duplicar renglón">⧉</button><button type="button" class="icon-btn purchase-remove" data-remove-purchase-item="${i}" aria-label="Quitar" title="Quitar">×</button></div>
    </div>`;
  }
  ...
  async function openPurchaseDialog(){await ensureCostingData(true);state.purchaseItems=[purchaseDefaultItem('shirt')];const f=qs('#purchaseForm');f.reset();f.elements.date.value=today();qs('#purchasePaymentMethod').value='cash';qs('#purchaseSurcharge').value='';renderPurchaseItems();qs('#purchaseDialog')?.showModal();}
  async function savePurchase(){
    ...
    const items=state.purchaseItems.map(x=>({materialType:x.materialType,name:String(x.name||'').trim(),quantity:Number(x.quantity)||0,color:String(x.color||'').trim(),size:String(x.size||'').trim(),fit:String(x.fit||'').trim(),materialClass:String(x.materialClass||'').trim(),gender:String(x.gender||'').trim(),widthCm:Number(x.widthCm)||0,unit:materialIsDtf(x.materialType)?'meter':'unit',unitPriceCents:pesosToCents(x.unitPricePesos)})).filter(x=>x.name&&x.quantity>0);if(!items.length)throw new Error('Agregá al menos un producto o material.');
  ...
  function productionMaterialLabel(m){return [m.name,m.fit,m.size,m.material_class,m.gender,m.color].filter(Boolean).join(' · ');}
  function productionMaterialCard(m){const selected=state.productionMaterialsSelected.some(x=>Number(x.materialId)===Number(m.id));return `<button type="button" class="production-picker-card ${selected?'selected':''}" data-add-production-material="${m.id}"><strong>${escapeHtml(m.name)}</strong><span>${escapeHtml([m.fit,m.size,m.material_class,m.gender,m.color].filter(Boolean).join(' · ')||materialTypeLabel(m.material_type))}</span><small>Disponible: ${Number(m.stock_qty).toLocaleString('es-AR',{maximumFractionDigits:3})} ${m.unit==='meter'?'m':'u'}</small><b>${money(m.average_cost_cents)} / ${m.unit==='meter'?'m':'u'}</b></button>`;}
  function renderProductionMaterialGallery(){const host=qs('#productionMaterialGallery');if(!host)return;const type=qs('#productionMaterialType')?.value||'shirt';const mats=state.materials.filter(m=>m.material_type===type&&Number(m.stock_qty)>0);host.innerHTML=mats.length?mats.map(productionMaterialCard).join(''):`<div class="notice">No hay ${escapeHtml(materialTypeLabel(type).toLowerCase())} con stock. Cargalo desde Compras.</div>`;}
  function renderProductionSelectedMaterials(){const host=qs('#productionSelectedMaterials');if(!host)return;host.innerHTML=state.productionMaterialsSelected.length?`<div class="production-selected-title">Materia prima elegida</div>${state.productionMaterialsSelected.map((x,i)=>{const m=state.materials.find(a=>Number(a.id)===Number(x.materialId));if(!m)return'';return `<div class="production-selected-row"><div><strong>${escapeHtml(productionMaterialLabel(m))}</strong><small>${money(m.average_cost_cents)} por ${m.unit==='meter'?'metro':'unidad'}</small></div><div class="purchase-stepper compact-stepper"><button type="button" data-prod-material-step="-1" data-index="${i}">−</button><input class="input" value="${Number(x.quantity)||1}" readonly><button type="button" data-prod-material-step="1" data-index="${i}">+</button></div><button class="icon-btn" type="button" data-remove-production-material="${i}">×</button></div>`}).join('')}`:'';calcProductionBuilderCost();}
  ...
  function renderProductionSelectedDesigns(){const host=qs('#productionSelectedDesigns');if(!host)return;host.innerHTML=state.productionDesignsSelected.length?`<div class="production-selected-title">Estampas elegidas</div>${state.productionDesignsSelected.map((x,i)=>{const d=state.designAssets.find(a=>Number(a.id)===Number(x.designAssetId));if(!d)return'';return `<div class="production-selected-row"><div><strong>${escapeHtml(d.name||d.file_name)}</strong><small>${Number(d.width_cm)||'?'} × ${Number(d.height_cm)||'?'} cm · ${d.print_material_type==='dtf_uv'?'DTF UV':d.print_material_type==='none'?'Sin DTF':'DTF textil'}</small></div><div class="purchase-stepper compact-stepper"><button type="button" data-prod-design-step="-1" data-index="${i}">−</button><input class="input" value="${Number(x.quantity)||1}" readonly><button type="button" data-prod-design-step="1" data-index="${i}">+</button></div><button class="icon-btn" type="button" data-remove-production-design="${i}">×</button></div>`}).join('')}`:'';calcProductionBuilderCost();}
  function productionBuilderCostData(){const finalQty=Math.max(1,Number(qs('#productionQuantity')?.value)||1),waste=Math.max(0,Number(qs('#productionWaste')?.value)||0);let materialCost=0,printCost=0;const lines=[],missing=[];for(const x of state.productionMaterialsSelected){const m=state.materials.find(a=>Number(a.id)===Number(x.materialId));if(!m)continue;const q=Math.max(.001,Number(x.quantity)||1),line=q*(Number(m.average_cost_cents)||0);materialCost+=line;const need=q*finalQty;if(Number(m.stock_qty)+1e-9<need)missing.push(`No alcanza ${m.name}: necesitás ${need.toLocaleString('es-AR',{maximumFractionDigits:3})} y hay ${Number(m.stock_qty).toLocaleString('es-AR',{maximumFractionDigits:3})}.`);lines.push(`${m.name}: ${money(line)}`)}for(const x of state.productionDesignsSelected){const d=state.designAssets.find(a=>Number(a.id)===Number(x.designAssetId));if(!d)continue;const type=d.print_material_type||'dtf_textile';if(type==='none')continue;const mat=[...state.materials].filter(m=>m.material_type===type).sort((a,b)=>(Number(b.stock_qty)||0)-(Number(a.stock_qty)||0))[0];if(!mat){missing.push(`Falta ${type==='dtf_uv'?'DTF UV':'DTF textil'} en Compras.`);continue}const width=Math.max(1,Number(mat.width_cm)||58),area=(Number(d.width_cm)||0)*(Number(d.height_cm)||0)*Math.max(1,Number(x.quantity)||1);if(!area){missing.push(`Faltan las medidas del diseño ${d.name}.`);continue}const meters=(area/(100*width))*(1+waste/100),line=meters*(Number(mat.average_cost_cents)||0);printCost+=line;if(Number(mat.stock_qty)+1e-9<meters*finalQty)missing.push(`No alcanza ${type==='dtf_uv'?'DTF UV':'DTF textil'} para ${d.name}.`);lines.push(`${d.name}: ${meters.toFixed(3)} m · ${money(line)}`)}const unitCost=Math.round(materialCost+printCost),sale=pesosToCents(qs('#productionSalePrice')?.value||0),profit=sale-unitCost,margin=sale>0?(profit/sale)*100:0;return {unitCost,materialCost:Math.round(materialCost),printCost:Math.round(printCost),sale,profit,margin,lines,missing,finalQty};}
  function calcProductionBuilderCost(){const host=qs('#productionBuilderCost');if(!host)return;const c=productionBuilderCostData();host.innerHTML=`<div><span>Costo materia prima</span><strong>${money(c.materialCost)}</strong></div><div><span>Costo estampas</span><strong>${money(c.printCost)}</strong></div><div class="production-cost-main"><span>Costo unitario estimado</span><strong>${money(c.unitCost)}</strong></div><div><span>Ganancia estimada por unidad</span><strong class="${c.profit>=0?'profit-positive':'profit-negative'}">${money(c.profit)}</strong><small>${c.sale>0?`${c.margin.toFixed(1)}% sobre venta`:'Ingresá un precio de venta'}</small></div><div><span>Ganancia estimada del lote</span><strong class="${c.profit>=0?'profit-positive':'profit-negative'}">${money(c.profit*c.finalQty)}</strong><small>${c.finalQty} unidad${c.finalQty===1?'':'es'}</small></div>${c.lines.length?`<small class="production-cost-lines">${c.lines.map(escapeHtml).join(' · ')}</small>`:''}${c.missing.length?`<div class="notice danger">${c.missing.map(escapeHtml).join('<br>')}</div>`:''}`;}
  ...
  function syncProductionPublicationDefaults(){const primary=productionPrimaryMaterial(),type=qs('#productionMaterialType')?.value||'shirt',cat=productionCategoryGuess(type),catSel=qs('#productionCategory');if(catSel&&!catSel.value&&cat)catSel.value=String(cat.id);if(primary){if(qs('#productionFit')&&!qs('#productionFit').value)qs('#productionFit').value=primary.fit||'';if(qs('#productionAudience')&&!qs('#productionAudience').value)qs('#productionAudience').value=primary.gender||'';}}
  ...
  async function openProductionDialog(){await Promise.all([ensureCategories(),ensureCostingData(true)]);if(!state.products.length){const d=await api('/api/admin/products');state.products=d.items||[]}state.productionMaterialsSelected=[];state.productionDesignsSelected=[];state.productionPriceDirty=false;state.mediaItems=[];state.newFiles=[];state.mediaHostId='productionMediaOrderList';qs('#productionQuantity').value='1';qs('#productionWaste').value='10';qs('#productionSalePrice').value='';qs('#productionNotes').value='';qs('#productionProductName').value='';qs('#productionCategory').innerHTML='<option value="">Elegir...</option>'+state.categories.map(c=>`<option value="${c.id}">${escapeHtml(c.name)}</option>`).join('');qs('#productionStatus').value='draft';['productionFit','productionAudience','productionComparePrice','productionDescription','productionMeaning','productionVerse','productionVerseReference','productionWeight','productionHeight','productionWidth','productionDepth'].forEach(id=>{const el=qs(`#${id}`);if(el)el.value=''});['productionIsNew','productionFeatured','productionBestseller'].forEach(id=>{const el=qs(`#${id}`);if(el)el.checked=false});renderProductionMaterialGallery();renderProductionSelectedMaterials();renderProductionDesignGallery();renderProductionSelectedDesigns();setProductionTab(1);qs('#productionDialog')?.showModal();}
  async function saveProductionBuilder(){const primary=productionPrimaryMaterial(),type=qs('#productionMaterialType')?.value||'shirt',qty=Math.max(1,Number(qs('#productionQuantity')?.value)||1),name=String(qs('#productionProductName')?.value||'').trim(),categoryId=Number(qs('#productionCategory')?.value),cost=productionBuilderCostData();if(!state.productionMaterialsSelected.length)throw new Error('Elegí al menos una materia prima.');if(cost.missing.length)throw new Error(cost.missing[0]);if(!name||!categoryId)throw new Error('Completá nombre y categoría en Publicación.');if(cost.sale<=0)throw new Error('Ingresá el precio de venta.');const recipe={baseMaterialType:'',baseMaterialName:'',wastePercent:Math.max(0,Number(qs('#productionWaste')?.value)||0),extraCostCents:0,designs:state.productionDesignsSelected.map(x=>({designAssetId:Number(x.designAssetId),quantity:Number(x.quantity)||1})),materials:state.productionMaterialsSelected.map(x=>({materialId:Number(x.materialId),quantity:Number(x.quantity)||1}))};const variant={color:primary?.color||'',size:materialIsGarment(type)?(primary?.size||''):'',stock:0,sku:''};const payload={name,category_id:categoryId,status:qs('#productionStatus')?.value||'draft',price_cents:cost.sale,compare_at_cents:pesosToCents(qs('#productionComparePrice')?.value||0),cost_cents:cost.unitCost,recipe,short_description:qs('#productionDescription')?.value||'',meaning_text:qs('#productionMeaning')?.value||'',verse_text:qs('#productionVerse')?.value||'',verse_reference:qs('#productionVerseReference')?.value||'',fit:primary?.fit||qs('#productionFit')?.value||'',audience:primary?.gender||qs('#productionAudience')?.value||'',sale_mode:'stock',inventory_stage:'to_print',garment_ready:1,print_ready:state.productionDesignsSelected.length?1:0,is_new:qs('#productionIsNew')?.checked?1:0,is_featured:qs('#productionFeatured')?.checked?1:0,is_bestseller:qs('#productionBestseller')?.checked?1:0,weight_grams:Number(qs('#productionWeight')?.value)||0,height_cm:Number(qs('#productionHeight')?.value)||0,width_cm:Number(qs('#productionWidth')?.value)||0,depth_cm:Number(qs('#productionDepth')?.value)||0,variants:[variant]};const d=await api('/api/admin/products',{method:'POST',body:JSON.stringify(payload)});const productId=Number(d.item.id);const orderedIds=[];for(const item of state.mediaItems){if(!item.file)continue;const f=new FormData();f.append('file',item.file);const uploaded=await api(`/api/admin/products/${productId}/media`,{method:'POST',body:f});orderedIds.push(Number(uploaded.id));}if(orderedIds.length)await api(`/api/admin/products/${productId}/media-order`,{method:'PATCH',body:JSON.stringify({ids:orderedIds})});const pd=await api(`/api/admin/products/${productId}`),variantId=Number(pd.item?.variants?.[0]?.id);if(!variantId)throw new Error('No se pudo crear la variante del producto.');await api('/api/admin/production',{method:'POST',body:JSON.stringify({productId,variantId,quantity:qty,notes:qs('#productionNotes')?.value||''})});state.costingLoaded=false;return productId;}
  ...
  function bind(){
    ...
    document.addEventListener('change',async e=>{
      ...
      if(e.target.id==='productionMaterialType'){renderProductionMaterialGallery();renderProductionDesignGallery();syncProductionPublicationDefaults();calcProductionBuilderCost();return}
    ...
    document.addEventListener('click',async e=>{
      ...
      if(e.target.id==='saveProductionBtn'){try{e.target.disabled=true;await saveProductionBuilder();qs('#productionDialog')?.close();toast('Producto creado y producción iniciada · materia prima descontada','success');await renderProduction()}catch(err){toast(err.message,'error')}finally{e.target.disabled=false}return}
