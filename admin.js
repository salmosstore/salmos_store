(() => {
  ...
  const titles={dashboard:'Dashboard',products:'Productos',orders:'Pedidos',customOrders:'Por Pedido',stock:'Stock',coupons:'Cupones',categories:'Categorías',expenses:'Caja',flyers:'Flyers',settings:'Configuración'};
  ...
  async function renderProducts(){
    ...
    qs('#adminContent').innerHTML=`
      ...
      <section class="settings-card bulk-price-card">
      ...
      <div class="notice">Orden de la tienda: arrastrá desde ☰ con el mouse o el dedo. Se guarda al soltar.</div>
      <div id="adminProductsTable">${productsTable(state.products)}</div>`;
    bindProductSort();
  }
  function productsTable(items){return `<div class="admin-card admin-table-wrap"><table class="admin-table"><thead><tr><th style="width:44px"></th><th style="width:38px">✓</th><th>Producto</th><th>Modalidad</th><th>Categoría</th><th>Precio</th><th>Stock</th><th>Estado</th><th></th></tr></thead><tbody id="sortableProducts">${items.length?items.map(p=>`<tr data-sort-product="${p.id}"><td><button class="drag-handle" type="button" aria-label="Mover producto">☰</button></td><td><input type="checkbox" data-bulk-product="${p.id}" aria-label="Seleccionar ${escapeHtml(p.name)}"></td><td><div style="display:flex;align-items:center;gap:10px">${p.primary_image_url?`<img class="mini-image" src="${escapeHtml(p.primary_image_url)}" alt="">`:'<div class="mini-image product-placeholder">S</div>'}<strong>${escapeHtml(p.name)}</strong></div></td><td><span class="status ${p.sale_mode==='order'?'warning':'success'}">${p.sale_mode==='order'?'Por pedido':'Stock'}</span></td><td>${escapeHtml(p.category_name||'')}</td><td>${money(p.price_cents)}</td><td>${p.available_stock}</td><td><span class="status ${p.status==='published'?'success':'warning'}">${p.status==='published'?'Publicado':p.status==='draft'?'Borrador':'Oculto'}</span></td><td><div class="admin-actions"><button class="btn btn-ghost" data-edit-product="${p.id}">Editar</button><button class="btn btn-ghost" data-duplicate-product="${p.id}">Duplicar</button></div></td></tr>`).join(''):`<tr><td colspan="9"><div class="empty-state"><strong>Todavía no hay productos.</strong>Creá el primero desde este panel.</div></td></tr>`}</tbody></table></div>`}
  ...
  async function renderCustomOrders(){
    ...
    qs('#adminContent').innerHTML=`<div class="admin-section-head"><div><h2>Pedidos por encargo y personalizados</h2><small class="muted">Los archivos se descargan en calidad original.</small></div></div>
      ...
      </article>`).join(''):'<div class="empty-state"><strong>Todavía no hay pedidos por encargo.</strong></div>'}</div>`;
  }
  ...
  function bind(){
    ...
    qs('#adminContent').addEventListener('input',e=>{if(e.target.id==='adminProductSearch'){const q=e.target.value.toLowerCase();const items=state.products.filter(p=>p.name.toLowerCase().includes(q)||String(p.category_name||'').toLowerCase().includes(q));qs('#adminProductsTable').innerHTML=productsTable(items);bindProductSort()}});
